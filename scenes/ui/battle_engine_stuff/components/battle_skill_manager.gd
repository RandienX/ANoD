		...
		for skill in all_skills:
			if skill:
				...
				skill_affordable.append(root.current_attacker.stats["mp"] >= skill.mana_cost 
										and (root.current_attacker.cannot_use_skills == false or skill.mana_cost == 0)
										and root.current_attacker.stats["hp"] >= skill.hp_cost 
										and root.tp >= skill.tp_cost)
...
func confirm_skill_target():
	...
	if skill.target_type == 0: #SingleEnemy
		...
		if target and target.stats["hp"] > 0:
			root.tp = max(0, root.tp - skill.tp_cost)
