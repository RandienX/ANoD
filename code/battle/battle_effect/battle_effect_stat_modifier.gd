...
@export var scaling_type: BattleEffect.ScalingType = BattleEffect.ScalingType.FLAT
...
func calculate_final_value(source: Entity, target: Entity) -> float:
	...
	match scaling_type:
		BattleEffect.ScalingType.NONE:
			pass
		BattleEffect.ScalingType.FLAT:
			pass  # value is already flat
		BattleEffect.ScalingType.PERCENT_BASE:
			var base_stat = target.stats[stat_key]
			final_value = base_stat * (value / 100.0)
		BattleEffect.ScalingType.PERCENT_CURRENT:
			var current_stat = target.get_effective_stat(stat_key)
			final_value = current_stat * (value / 100.0)
		BattleEffect.ScalingType.LEVEL_SCALE:
			final_value = value * source.level
		BattleEffect.ScalingType.STAT_SCALE:
			if scale_stat != "":
				var scale_value = source.stats[scale_stat]
